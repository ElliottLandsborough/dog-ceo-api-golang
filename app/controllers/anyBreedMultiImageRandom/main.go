...
func handler(request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	rootPrefixes := aws.PrefixesToSlice(aws.GetObjectsByDelimeterAndPrefix(svc, bucket, "/", ""))
	randomPrefix := breed.GetRandomItemFromSliceString(rootPrefixes)
	slice := breed.PrependStringToAllSliceStrings(aws.ObjectsToSlice(aws.GetObjectsByDelimeterAndPrefix(svc, bucket, "", randomPrefix)), os.Getenv("CDN_DOMAIN_PREFIX"))
